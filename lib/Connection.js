...
function Connection (socket) {
  ...
  this.queue.on('add', this.sendPending.bind(this))
  this.on('ack', this.sendPending.bind(this))
  this.interval = setInterval(this.sendPending.bind(this), 2000)
...
Connection.prototype.dryBuffer = function() {
  if(!this.sent) return
  if(!this.queue[0] || this.queue[0][1] != this.sent) this.queue.unshift(['edit', this.sent])
  this.awaitingAck = false
}

Connection.prototype.resetBuffer = function() {
  this.queue = new Queue
  this.awaitingAck = false
  this.sent = false
  this.queue.on('add', this.sendPending.bind(this))
...
Connection.prototype.sendPending = function () {
  if(this.awaitingAck) return
  if(!this.queue.length) return
  
  var ev = this.queue.shift()
  this.socket.emit.apply(this.socket, ev)
